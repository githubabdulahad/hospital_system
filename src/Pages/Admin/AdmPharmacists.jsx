...
const AdmPharmacists = () => {
  ...
  const filteredPharmacists = pharmacists.filter(pharm =>
    pharm.name.toLowerCase().includes(search.toLowerCase()) ||
    pharm.email.toLowerCase().includes(search.toLowerCase()) ||
    pharm.address.toLowerCase().includes(search.toLowerCase()) ||
    pharm.phone.toLowerCase().includes(search.toLowerCase())
  );
  ...
  return (
    <div className="p-6" style={{ fontFamily: "'Gill Sans MT', 'Gill Sans', 'GillSans', 'Arial', 'sans-serif'" }}>
      <div className="flex items-center mb-6">
        ...
        <h2 className="text-3xl font-semibold text-gray-500">Pharmacists</h2>
      </div>
      <div className="flex justify-between items-center mb-4">
        <div></div>
        <CommonAddButton label="Add Pharmacist" />
      </div>
      <CommonTable
        columns={[ 
        ...
        data={filteredPharmacists.map(pharm => ({ ...pharm, profile: true }))}
        actions={[
          { label: <button 
      ...
      <div className="flex justify-between items-center mt-4 text-gray-500 text-xs">
        <span>Showing 1 to {filteredPharmacists.length} of {filteredPharmacists.length} entries</span>
