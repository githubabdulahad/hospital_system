...
const navItems = [
  ...
  { label: 'Prescription', to: 'prescriptions', icon: (
  ...
  { label: 'Admit History', to: 'admit-history', icon: (
  ...
  { label: 'Operation History', to: 'operation-history', icon: (
...
const appointmentDropdownItems = [
  { label: 'Appointment List', to: 'appointment-list' },
  { label: 'Pending Appointments', to: 'pending-appointments' },
];
...
const SidebarDropdown = ({ label, icon, children, to, open, setOpen }) => {
  ...
  return (
    <div>
      <div className="flex items-center w-full px-4 py-2 rounded hover:bg-[#C0E6DA] hover:text-[#232946]">
        <NavLink
          to={to}
        ...
        >
          <svg className={`w-4 h-4 transition-transform ${open === label ? 'rotate-90' : ''}`} fill="none" stroke="currentColor" strokeWidth="2" viewBox="0 0 24 24"><path d="M9 5l7 7-7 7" /></svg>
        </button>
      </div>
      {open === label && (
...
const PatientSidebar = () => {
  ...
  return (
    <aside className="w-70 bg-[#232946] text-white flex flex-col shadow-lg sticky top-0 overflow-y-auto" style={{ fontFamily: "'Gill Sans MT', 'Gill Sans', 'GillSans', 'Arial', 'sans-serif'", height: 'calc(100vh - 120px)' }}>
  ...
  label="Appointment"
  icon={
    <svg className="w-5 h-5 inline-block mr-2" fill="none" stroke="currentColor" strokeWidth="2" viewBox="0 0 24 24"><path d="M8 7V3m8 4V3m-9 8h10M5 21h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v12a2 2 0 002 2z" /></svg>
  }
  to="appointments"
  open={openDropdown}
