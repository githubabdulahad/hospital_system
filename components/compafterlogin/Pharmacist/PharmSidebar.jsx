...
const navItems = [
  ...
  { label: 'Medical Category', to: '/pharmacist/medicine-category', icon: (
  ...
  { label: 'Payroll', to: '/pharmacist/payroll', icon: (
  ...
  { label: 'Profile', to: '/pharmacist/profile', icon: (
...
const SidebarDropdown = ({ label, icon, children, open, setOpen }) => {
  ...
  // Check if any of the dropdown children are active
  const isDropdownActive = children.some(child => 
  ...
  return (
    <div>
      <div className={`flex items-center w-full px-4 py-2 rounded ${
        isDropdownActive ? 'bg-[#C0E6DA] text-[#232946]' : 'hover:bg-[#C0E6DA] hover:text-[#232946]'
      }`}>
        ...
        <button
          ...
          className="ml-2 p-1 rounded focus:outline-none hover:bg-black/10 transition-colors"
          type="button"
        >
          <svg 
            className={`w-4 h-4 transition-transform duration-200 ${open === label ? 'rotate-90' : ''}`} 
            fill="none" 
            stroke="currentColor" 
            strokeWidth="2" 
            viewBox="0 0 24 24"
  ...
  return (
    ...
    >
      <div className="flex flex-col items-center py-8 border-b border-gray-700 px-3">
        ...
        <div className="text-lg font-semibold text-center">Welcome, Pharmacist</div>
      </div>
