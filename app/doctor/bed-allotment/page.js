...
const BedAllotment = () => {
  ...
  const handleDischargeBed = (bed) => {
    ...
    if (window.confirm(`Are you sure you want to discharge bed ${bed.bedNumber}?`)) {
      ...
      setBeds(updatedBeds);
      showToast(`Bed ${bed.bedNumber} has been discharged successfully`, "success");
    }
  };
  ...
  const handleAllotmentSubmit = (e) => {
    ...
    showToast(`Bed ${selectedBed.bedNumber} allotted to ${allotmentData.patientName}`, "success");
  };
  ...
  const handleAddBedSubmit = (e) => {
    ...
    const bedExists = beds.some(bed => bed.bedNumber === newBedData.bedNumber);
    if (bedExists) {
  ...
  return (
    <div className="p-6" style={{ fontFamily: "'Gill Sans MT', 'Gill Sans', 'GillSans', 'Arial', 'sans-serif'" }}>
      ...
      {/* Allotment Modal */}
      {showAllotModal && (
        <AllotbedModal selectedBed={selectedBed} setShowAllotModal={setShowAllotModal} handleAllotmentSubmit={handleAllotmentSubmit} handleInputChange={handleInputChange} allotmentData={allotmentData} />
      )}

      {/* Add Bed Modal */}
      {showAddBedModal && (
        <AddbedModal setShowAddBedModal={setShowAddBedModal} handleAddBedSubmit={handleAddBedSubmit} newBedData={newBedData} handleNewBedInputChange={handleNewBedInputChange}/>
      )}
    </div>
  );
};
